...
/// produce
///
/// Create a rust kafka producer client with ``rust-rdkafka`` and include paths to
/// client tls assets based off environment variables. This producer will connect
/// to the ``brokers`` addresses (format: ``fqdn1:port,fqdn2:port,fqdn3:port``)
/// and publish messages to the ``topic`` within the kafka ``brokers`` list
///
...
/// * `topic` - publish messages to this topic within the kafka ``brokers``
///
/// # Examples
///
/// ```bash
